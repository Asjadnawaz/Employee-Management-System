...
async function addEmployee() {
    ...
    if (isNaN(id) || isNaN(salary)) {
        console.log("Invalid input. Please enter numeric values for ID and Salary.");
    }
    else {
        let details = {
            employeeId: id,
...
async function deleteEmployee() {
    ...
    let index = collectionOfEmployee.findIndex(employee => employee.employeeId === id);
    if (index !== -1) {
...
async function updateEmployee() {
    ...
    let employee = collectionOfEmployee.find(employee => employee.employeeId === id);
    if (employee) {
        let nameInput = await UserInput(`Update Employee Name (current: ${employee.employeeName}): `);
        let salaryInput = await UserInput(`Update Employee Salary (current: ${employee.salary}): `);
        ...
        if (!isNaN(salary)) {
            ...
            console.log("Invalid salary input. Please enter a numeric value for salary.");
        }
    }
    else {
        console.log("Employee not found.");
